...
const Projects: React.FC<ProjectsProps> = ({ reverse = false, title, description, imageSrc, featuredText, icons }) => {
    return (
        <div className="relative flex">
            <div className={`absolute top-[-40px] ${reverse ? 'left-[-10px]' : 'right-[-50px]'} w-95 h-95 rounded-full bg-gradient-to-r from-[#763CAC]/40 to-[#320F85]/40 blur-lg opacity-90 -z-10`}></div>
            <div className={`flex ${reverse ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-[657px] h-auto flex flex-row z-10 relative ${reverse ? 'pl-[230px]' : 'left-[70px]'}`}>
                ...
                <div className={`relative w-[450px] h-[300px] bg-[#1e012c] rounded-[10px] overflow-hidden z-0 flex items-end pt-[10px] ${reverse ? 'pr-[30px]' : 'pl-[20px]'} ${reverse ? 'ml-[60px]' : 'ml-[0]'}`}>
                    <div className={`relative pt-4 w-full h-full overflow-hidden ${reverse ? 'pr-4' : 'pl-4'}`}>
                        <img src={imageSrc} className={`w-[500px] h-[300px] object-cover ${reverse ? 'rounded-tr-[16px]' : 'rounded-tl-[16px]'}`} />
                    </div>
                </div>
            </div>
        </div>
